...
contract LimePoolV2 is Ownable {
    using SafeMath for uint256;
    ...
    // ALLOY tokens created per block.
    ...
    // The block number when PMP mining starts.
    uint256 public startBlock;
    // The block number when PMP mining ends.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 drugsReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accAlloyPerShare = accAlloyPerShare.add(drugsReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 drugsReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        pool.accAlloyPerShare = pool.accAlloyPerShare.add(drugsReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    // Withdraw PMP tokens from STAKING.
